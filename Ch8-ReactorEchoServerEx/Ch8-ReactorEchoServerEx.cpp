...
public:
  Session(Poco::Net::StreamSocket& socket, Poco::Net::SocketReactor& reactor) : m_Socket(socket), m_Reactor(reactor) {
    ...
    std::cout << "connection from " << m_PeerAddress << " ..." << std::endl;                                // 데이터 읽기 가능할 때 발생하는 이벤트
    ...
    // Poco::Net::select 호출 시 react 할 소켓이 없는 경 우 발생하는 이벤트
...
int main() {
  ...
  reactor.run();
  std::cout << "Reactor: 종료..." << std::endl;        getchar();        return 0;
}
